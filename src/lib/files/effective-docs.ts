...
/** Canonical document identity for a file name (extension + split suffix stripped). */
export function documentKey(fileName: string): string {
  const stem = fileName.replace(/\.[a-z0-9]{1,5}$/i, "");
  return stem.replace(SPLIT_SUFFIX, "").trim().toLowerCase();
}

/** Collapse a raw upload list to effective documents:
 * 1. exact duplicates (same name + byte size) count once;
...
export function effectiveDocuments<T extends FileLike>(files: T[]): T[] {
  ...
  for (const f of files) {
    const k = `${f.file_name.toLowerCase()}|${f.file_size}`;
