...
export function Id3UploadForm({ savedRulesCount = 0 }: { savedRulesCount?: number }) {
  ...
  const [useSavedRules, setUseSavedRules] = useState(savedRulesCount > 0);

  function submit(formData: FormData) {
    formData.set("engine_id", "id3_validation");
    if (useSavedRules) formData.set("use_saved_rules", "1");
        ...
        <div className="space-y-1.5">
          ...
          {savedRulesCount > 0 && (
            <label className="flex items-center gap-2 text-xs">
              ...
              />
              Use the saved rules ({savedRulesCount} rule{savedRulesCount === 1 ? "" : "s"} from
              the Rules tab)
            </label>
          )}
          {!useSavedRules && (
