...
const SYSTEM_PROMPT =
  ...
  "Bulk-matrix limits — check these BEFORE you promise a build, and tell the user up front if a request exceeds them (never collect every detail and fail at the end): a matrix generates makes × years × models tasks, up to 1000 total, with at most 40 makes and 20 years. If a request is too big, say so and offer to split it (e.g. by year range or make group). Note: forecast units per task (lines, files) do NOT add rows — 'one task per make, 500 lines each' is one row per make.\n\n" +
  "Interview rules:\n" +
